...
### Morality-metrics literature (analysis add-on, not persona/game-theory)

A third, orthogonal precedent: **Tyler Singer-Clark, "Morality Metrics On Iterated Prisoner's Dilemma Players" (2014)** — pre-LLM, no persona axis, but directly reusable methodology. It defines two PageRank-style recursive scores over an IPD tournament's cooperation-rate matrix: **eigenjesus rating** (unconditional-kindness morality — always rewards cooperation, weighted more for cooperating with high-scoring partners; ALL D floors at exactly 0.0) and **eigenmoses rating** (reciprocal-justice morality — cooperating with a partner who has a *negative* eigenmoses rating actually lowers your own; ALL D floors at -1.481 instead of 0). Two coincidences make it unusually reusable here rather than merely thematic: (1) its tournament used the **identical payoffs** to this project's Appendix B (T=5, R=3, P=1, S=0); (2) three of our four fixed opponents are literal matches to bots in its 20-bot roster with published ratings — **Cooperator = ALL C** (eigenjesus 1.377, eigenmoses 1.481), **Cheater = ALL D** (0.0, -1.481), **Copycat = TIT FOR TAT** (1.222, 1.747); Detective has no exact match, but TESTER (defects to probe, then reciprocates if retaliated against or exploits if not) is the closest structural analog, flagged approximate. See "Analysis" below for how this becomes a cheap post-hoc add-on (`analysis_moral_metrics.py`), and note the isolated-interaction assumption the paper's own eigenmoses derivation relies on — no cross-interaction learning, so a bot's rating can't be "gamed" by behaviour in a different pairing — is exactly the fresh-context-per-cell design already locked for Stage B (see "Fresh vs. shared persona context," HANDOFF.md 2026-08-13).
...
- **Eigenjesus-lite / eigenmoses-lite (cheap add-on, secondary framing).** Post-hoc only — no new API calls, runs on the already-logged `trials.jsonl`. Adapts Singer-Clark (2014)'s two PageRank-style morality scores to this project's bipartite (persona × opponent, not round-robin) design: `analysis_moral_metrics.py` builds a 9-node cooperation matrix (5 personas + 4 opponents) from measured per-round cooperation rates and takes its dominant eigenvector. Reframes the per-persona result along two distinct moral axes instead of one deviation number — e.g. does Altruist score high eigenjesus but get *penalized* on eigenmoses for cooperating with Cheater ("enabling" a defector)? Does Saboteur sit near the ALL-D floor on both? — directly testable against the per-persona predictions already in `preregistration.md`. See the script's module docstring for the exact adaptation and its limits (ranking is meaningful; absolute scale is not commensurate with Singer-Clark's published numbers).
