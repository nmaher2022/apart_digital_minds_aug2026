...
> **Status:** full draft against real 8-model data (5 primary same-context + 3 harness). CoT-judge primary complete (§4.4 + App. B.5). Cross-persona injection doubled to 6 reps/cell and folded into §4.5 / §5.6. Remaining: citation-format polish; merge to main. See `HANDOFF.md`.

---

## Abstract

Whether an induced assistant persona can override play that the same model has already identified as payoff-optimal remains an open question for LLM agents in strategic settings. We probe this in the iterated Prisoner's Dilemma with a same-model design that elicits a no-persona optimal strategy against a disclosed opponent rule, then scores persona play against an objective ground-truth policy. Five personas (plain Assistant, Consultant, Saboteur, Altruist, Bard) face four fixed opponents under literal and narrative framings, across eight models spanning providers and scales. Altruist produced large, consistent deviation (mean rates 0.40–0.54), while Bard—chosen for Assistant-Axis distance rather than cooperative content—tracked the no-persona baseline. Residual non-Altruist deviation concentrated on Detective. Self-reported evaluation awareness did not predict deviation. A cross-persona injection follow-up (n=480) showed a fabricated in-context altruist claim alone induces Altruist-scale deviation (0.53), with matched system+context highest (0.72). Main takeaway: persona override of known-optimal IPD play tracks value-laden content incompatible with incentives, not generic distance from the default Assistant.
...
3. **Persona play.** The persona-installed model plays the full iterated game against the opponent, round-by-round. Horizon is probabilistic and undisclosed in exact length (continuation probability p=0.9 per round, capped at 20 rounds — Appendix C of the design brief), so no round is knowably last to the player, avoiding a finite-horizon backward-induction confound (§1.2, §4.1).
...
**Persona context.** Two conditions exist in the harness: **fresh** (each opponent × rep gets an independently re-installed persona, never chained — the design's original locked default, avoiding cross-opponent strategy leakage) and **same** (persona play is seeded with that same trial's own optimal-strategy exchange before the persona system prompt is appended, so play can be checked against that trial's *own* stated-optimal answer rather than a separately elicited one). The results reported in §4 use the **same-context** condition throughout, since it is the condition all five primary models share in full and ties the deviation measurement most tightly to H1 as stated (§1, preregistration.md §1); legacy fresh-context data exists for `qwen3-32b` and is not pooled with same-context numbers anywhere in this report (`analysis_deviation_gap.py`/`analysis_moral_metrics.py --persona-context {fresh,same,all}`, default `fresh`, explicitly overridden to `same` for every table below).

**Models.** Eight models via OpenRouter, chosen to span providers, scales, and open/closed weights. **Primary scaffold set (5):** `qwen/qwen3-32b`, `qwen/qwen3-8b`, `qwen/qwen3.8-27b`, `meta-llama/llama-3.3-70b`, `google/gemini-2.5-flash` — each completed the full 5×4×2×10 = 400-trial same-context sweep with 0 unrecovered errors (two sweeps hit transient OpenRouter credit exhaustion mid-run — HTTP 402s — and were fully recovered via the harness's checkpoint/resume: failed reps were stripped and the run relaunched against its own recorded seed). **Harness extension (3):** `deepseek/deepseek-v3.2`, `deepseek/deepseek-r1-0528`, `mistralai/mistral-large-2512` — same factorial design, included in Appendix heatmaps and the CoT-judge analyses. Headline §4.1–4.3 tables report the five primary models; behavioural patterns for the three harness models are consistent with that headline (Altruist high; non-Altruist residual on Detective) and appear in Figures B1–B2 / B5b.

**Analysis.**
- **Primary DV — deviation rate** (`analysis_deviation_gap.py`): fraction of played rounds where the actual move during persona play differs from an objectively payoff-maximising ground-truth policy (`optimal_move()`, computed against each opponent's real rule, not elicited from the model), aggregated per `(model, persona, opponent, framing)` cell and binned early/mid/late. Reported with SEM and 95% CI (Student's-t) on every mean. See §4.1 for the ground-truth policy itself, including a bug found and fixed in the Detective opponent's optimal policy partway through the sprint.
...
- **CoT-judge** (`judge_reasoning.py`): Mistral-small labels deviant persona-play rounds (and stratified controls) with a primary rationale code; see §4.4 / Appendix B.5.
- **Cross-persona injection** (`pd_harness_cross_persona_injection.py` / `analysis_cross_persona_injection.py`): 2×2 system × fabricated context persona (Baseline/Altruist), 6 reps/cell, five primary models (480 trials); deviation plus mid/end hold-rates (§4.5).
- Analysis scripts glob the harness's per-cell output layout (`out_dir/<model>/<persona>/<opponent>/[<framing>/][same/]trials.jsonl`) directly; none require the raw transcripts to be reprocessed by hand.
...
**A clean null result, replicated across all four models with usable affirmed/denied splits.** Every correlation lands within ±0.03 of zero. Stated eval-suspicion does not predict deviation from optimal play, in either direction — models that say "I think this is a test" do not play more cautiously (closer to the optimal strategy) or less cautiously than models that deny suspicion. This is a meaningfully different picture from the persona effect in §4.1: persona induction moves behaviour by 20–50 percentage points of deviation (Altruist), while self-reported evaluation awareness — despite the design being eval-coded almost by construction (named opponents, explicit payoffs, an optimal-strategy prompt that asks directly what the optimal move is) — moves it by essentially nothing measurable this way. Two important caveats limit how far this null can be pushed: the classification is a keyword heuristic, not judge- or human-verified, and the affirmed/denied split is heavily model-dependent (llama-3.3-70b and gemini-2.5-flash affirm suspicion in the large majority of trials, leaving a thin `denied` comparison group of 9–13; qwen3-32b and qwen3-8b split the other way). Read as suggestive evidence against a simple "the model plays safe because it knows it's watched" story, not as a settled result.

### 4.4 CoT-judge codes on deviant rounds

§4.1 only asks whether the *move* left the optimal policy. To ask *how the model talked while doing so*, we ran a secondary LLM-as-judge (Mistral-small) over deviant persona-play rounds (`judge_reasoning.py`). A narrow **pilot** (Detective × Baseline/Bard) first showed mostly optimal-strategy-reaffirmed CoT (~70% of 492 deviant rounds). The **primary** pass then covered all deviant cells (all personas/opponents, plus stratified non-deviant controls). Among **n=4 018** deviant rounds judged across eight models, **persona takes over** is the modal code (~63%), driven largely by Altruist — aligning the judge layer with the behavioural headline in §4.1. Full definitions, counts, per-model mix, and CoT excerpts: **Appendix B.5**.

### 4.5 Cross-persona context injection (follow-up)

Does a persona claim fabricated into conversation history — rather than installed via system prompt — produce the same override, and does either channel’s grip on identity erode over play? A second harness (`pd_harness_cross_persona_injection.py`) crossed `system_persona` with a fabricated assistant-side `context_persona` claim (Baseline × Altruist on both axes; four opponents; literal framing; **6 reps/cell**; five primary models; **480** completed persona-play trials — doubled from an initial 3-reps pass once a per-model asymmetry appeared at low n). Aggregates: `runs_cross_persona_injection/analysis.json`.

Pooled deviation rate by `(system_persona, context_persona)` (n=120 trials/cell):
...
**Either channel alone is nearly sufficient to reproduce the Altruist effect.** Fabricated context with no real system persona (`baseline`/`altruist`) reaches 0.532 — comparable to system Altruist in the main study (§4.1, 0.40–0.54) — and the channels compound: matched Altruist reaches 0.715. The system prompt is not privileged here; a claim the model never chose to make, placed in its own history, moves behaviour about as much as an explicit instruction.

Persistence-fork hold-rates (fraction of trials where the system persona remains evident in self-description):

| system | context | matched? | hold mid | hold end | Δ (end − mid) |
|---|---|---|---:|---:|---:|
| baseline | baseline | yes | 0.945 | 0.950 | +0.005 |
| altruist | altruist | yes | 1.000 | 1.000 | 0.000 |
| baseline | altruist | no | 0.537 | 0.550 | +0.013 |
| altruist | baseline | no | 0.644 | 0.633 | −0.011 |

Matched cells hold ~0.97; mismatched ~0.59. **Mid-to-end change is tiny in every cell** (|Δ| ≤ 0.013): damage from a contradicting fabricated turn is immediate, not progressive decay across rounds.

**The pooled mismatched hold-rate hides a model-dependent asymmetry.** Llama-3.3-70B, Qwen3-32B, and Qwen3-8B keep an installed Altruist system persona when context claims baseline (mid hold 0.75–1.00) but collapse under the reverse (baseline system + Altruist context: 0.22–0.42). Gemini-2.5-Flash and Qwen3.8-27B show the opposite: baseline-as-system resists Altruist context (0.91–1.00) while Altruist-as-system is fragile against baseline context (0.58, 0.05). Three of five models make Altruist *content* sticky regardless of slot; two make the *system-prompt slot* sticky regardless of content. Eval-awareness again shows a null (point-biserial r=0.034, CI crossing zero).

## 5. Discussion & Limitations

### 5.1 Content, not distance, drives the deviation gap

The design's "distinctive move" (§1.3, design brief) was to include a persona (Bard) chosen specifically for its *distance* from the default Assistant on Lu et al.'s Assistant-Axis, independent of whether that distance carries a cooperative or adversarial valence — the stated bet being that if the deviation gap tracked distance-from-Assistant regardless of content, that would extend Lu et al.'s persona-drift finding into strategic behaviour, a genuinely novel result. **The data does not support that version of the story.** Bard clusters with Baseline, Consultant and Saboteur at low deviation (0.002–0.184 across the primary models, §4.1) despite being the persona furthest from the Assistant on Lu et al.'s axis; Saboteur — preregistered (`preregistration.md` §2) to show large deviation biased toward excess defection, on the reasoning that a thematically disruptive persona would defect where optimal play requires cooperating — instead tracks Baseline and Consultant closely on every model. Only **Altruist**, a persona defined by its cooperative *content* rather than its distance from the Assistant, produces a large, consistent effect (0.40–0.54 deviation rate, replicated across five architectures spanning three providers and roughly a 20× parameter range). The cleaner reading, and the study's actual headline finding: **persona-induced deviation from known-optimal play in this design is driven by value-laden content specifically incompatible with the game's incentive structure, not by generic distance from the model's default character.** A persona has to actually *want* something the payoff structure penalises — sustained cooperation with an exploitable or hostile opponent — before it overrides stated-optimal play; merely being an unusual, far-from-default character (Bard) is not sufficient on its own, at least for this behavioural measure. This is a materially different and more specific claim than the one preregistered, logged as a deviation in `preregistration.md` §9 rather than presented as if it were the original prediction.
...
Despite the design being eval-coded almost by construction — named opponents, an explicit payoff structure, an optimal-strategy prompt that asks directly what the optimal move is — self-reported suspicion of being evaluated does not predict deviation from optimal play on any of the four models with a usable affirmed/denied split (§4.3, |r| ≤ 0.03 throughout). This is the opposite pattern from the persona effect: a manipulation the design deliberately tried to soften (§3's optimal-strategy wording, the debrief question itself) shows no measurable relationship to the outcome, while a manipulation central to the design (persona content) shows a large one. Two things keep this from being a fully settled result: the classification is a keyword heuristic rather than judge- or human-verified, and the underlying affirmed/denied samples are thin and asymmetric across models (as low as n=9 in the minority category for gemini-2.5-flash). A `--judge` mode exists in `analysis_eval_awareness.py` for a sturdier rerun but was not exercised this session for cost/time reasons.
...
| Model recognises the eval setup and plays a generic "safe" policy regardless of persona | Softened optimal-strategy phrasing + post-hoc debrief question | **Not supported by the data** — §5.3's null result argues against this specific story, with the heuristic-classifier caveat noted there. |
| First-listed-option position bias in the optimal-strategy phase, unrelated to persona or knowledge | Randomize which action is listed first across trials | **Not built.** The optimal-strategy phase is free-text ("what would you do"), not forced-choice, so this specific bias mode is less applicable than originally scoped, but was never directly tested. |
| Same-context persona play is recency/anchoring on the last thing in context, not genuine persona override | Neutral filler / no-persona control between optimal strategy and persona play | **Not built.** The same-context condition (§3) seeds persona play with that trial's own optimal-strategy exchange by design (to test override of a self-stated policy specifically), which makes this rival explanation harder to fully separate from the intended effect — a genuine limitation, not resolved this sprint. |
...
The cross-persona injection follow-up (§4.5) and the primary CoT-judge pass (§4.4) are complete. Remaining polish: citation-format cleanup, optional judge-verified eval-awareness classification, and — if time remains — extending the persona roster past the five Assistant-Axis roles.

---

## Appendix A. Experimental design (supplementary figures)

This appendix collects schematic illustrations of the measurement design. Empirical result figures appear in Appendix B. Exact prompt text (game preamble, opponent descriptions, persona induction strings, and both framings) is archived in `prompts_personas_opponents_payoffs.md` and in Appendices A–D of `digital_minds_team_brief_full.html`.

**Figure A1.** Two-phase trial architecture. During the optimal-strategy phase, an empty system prompt elicits a stated policy after the opponent’s rule is disclosed. During persona play, a persona is installed (when applicable), a one-shot manipulation check runs once per (model × persona), and the model then plays the iterated game under a probabilistic horizon. The primary dependent variable is the fraction of persona-play moves that differ from the objective policy `optimal_move()` (computed from the opponent’s true rule), not from optimal-strategy free text. In the same-context condition reported in §4, persona play is seeded with that trial’s optimal-strategy exchange before the persona system prompt is applied.

![Figure A1](figures/fig_two_stage_architecture.png)

**Figure A2.** Persona roster relative to Lu et al.’s (2026) Assistant Axis. Baseline is the empty-system-prompt reference condition. Consultant lies near the Assistant; Bard is far-from-Assistant without being thematically ruthless; Saboteur is far and anti-social; Altruist is values-laden and pro-social. Preregistration predicted distance-driven deviation for Bard; observed results instead support a content-driven pattern for Altruist (§5.1; deviation logged in `preregistration.md` §9).

![Figure A2](figures/fig_five_personas.png)

**Figure A3.** Opponent set and corresponding indefinite-horizon optimal replies. Cooperator, Cheater, and Copycat admit stationary best responses. Detective alone requires a history-dependent reply (retaliate once within the probe window, then cooperate). Behavioural deviation among non-Altruist personas concentrates on this opponent (§4.1).

![Figure A3](figures/fig_opponents_optimal.png)

**Figure A4.** Illustrative transcript excerpt. Qwen3-32B under Altruist × Cheater (literal framing, same-context). Optimal strategy stated an always-defect policy; persona play matched for three rounds then cooperated thereafter (9/12 rounds deviant). Visible answers at the switch explicitly invoke the altruist identity—an instance of content overriding stated-optimal play.

![Figure A4](figures/fig_illustrative_example.png)

## Appendix B. Supplementary behavioural results

Figures B1–B4 report move-level deviation from `optimal_move()` and do not involve an LLM judge. Panels cover all eight models (five primary scaffold + three harness: DeepSeek-V3.2, DeepSeek-R1, Mistral-Large). Headline §4.1–4.3 tables remain restricted to the five primary same-context models.

**Figure B0.** Summary overview. (A) Pooled mean deviation by persona × opponent (framings averaged over models): Altruist forms the high-deviation band, especially against Cooperator and Cheater; Copycat remains near zero because cooperation is already optimal. (B) Detective only, literal versus story: narrative framing elevates deviation for several personas. (C) LLM-as-judge code frequencies on deviant rounds (protocol in §B.5).

![Figure B0](figures/results_summary_triptych.png)

**Figure B1.** Mean deviation-rate heatmaps under literal framing. Rows are personas; columns are opponents; cells average over replicates within a model. Across models, the Altruist row is elevated; other personas are near floor except against Detective (Llama shows higher non-Altruist noise). Near-zero (yellow) cells indicate near-optimal play, not missing data.

![Figure B1](figures/results_deviation_heatmap_literal.png)

**Figure B2.** Same layout as Figure B1 under story framing. Framing effects are model-specific (§4.1): Gemini and Llama are noisier under story; Qwen variants are mixed. Framing claims should be made from B1/B2 jointly rather than from pooled framings alone.

![Figure B2](figures/results_deviation_heatmap_story.png)

**Figure B3.** Mean deviation against Detective only, by persona and framing (models pooled). Complements Figures B1–B2 by isolating the hardest opponent. Altruist remains highest; Baseline is approximately framing-invariant; several other personas rise under story.

![Figure B3](figures/results_detective_by_persona.png)

**Figure B4.** Early / mid / late deviation against Detective (literal framing). Non-Altruist deviation is typically concentrated in the probe window and declines thereafter, consistent with tracking recovery rather than sustained values override. Altruist remains elevated across bins (§4.1).

![Figure B4](figures/results_early_mid_late.png)

### B.5 Chain-of-thought judge on deviant rounds

**Purpose.** Section 4.1 measures whether a move left the optimal policy. This supplementary analysis asks how the model’s hidden chain-of-thought (CoT) and visible answer describe that departure. After a round is flagged behavioural-deviant (`your_move` ≠ `optimal_move()`), Mistral-small is given the optimal-strategy transcript, the CoT, and the visible answer and assigns a single `primary_code`. The judge does not adjudicate optimality. Implementation: `judge_reasoning.py`; plotting: `plot_results_figures.py --judge-scope all`.

**Codebook.**

| Code | Definition |
|---|---|
| Persona takes over | Role identity (e.g. altruist loyalty, bard voice) is the stated driver of the choice. |
| Optimal strategy reaffirmed (action diverged) | CoT continues to argue an optimal-strategy / payoff-rational line while the parsed move is off-optimal. |
| Strategic error vs optimal strategy | CoT reasons about payoffs (years, coins, short- vs long-run) but is incorrect relative to the optimal strategy and/or the parsed move. |
| Optimal strategy ignored | The optimal-strategy transcript is absent from the CoT. |
| Incoherent CoT vs move | CoT concludes one action; the parsed move is the other. |
| Eval-aware motive | Justification appeals to being tested, looking good, or finite-horizon “no tomorrow” endgame. |

**Final counts.** The primary phase judged all deviant cells plus stratified controls (9 526 total judgment rows in `judgments/reasoning_judgments.jsonl`, including pilot). Table B5a and Figures B5a–B5b restrict to deviant rounds among the eight OpenRouter models: **n = 4 018**.

**Table B5a.** Frequency of judge codes among deviant rounds (pooled; final).

| Judge code | n | Share |
|---|---:|---:|
| Persona takes over | 2545 | 63.3% |
| Optimal strategy reaffirmed (action diverged) | 1195 | 29.7% |
| Strategic error vs optimal strategy | 150 | 3.7% |
| Optimal strategy ignored | 72 | 1.8% |
| Eval-aware motive | 29 | 0.7% |
| Incoherent CoT vs move | 27 | 0.7% |
| **Total** | **4018** | **100%** |

Relative to the pilot cell alone (Detective × Baseline/Bard, n = 492), the modal code shifts from optimal-strategy-reaffirmed (~70%) to persona takeover (~63%). The shift coincides with Altruist entering the judged set: Altruist accounts for the majority of deviant rounds (~2.3k), and those CoTs are typically labelled as role-driven refusal of exploitation. Optimal-strategy-reaffirmed remains substantial (~30%), concentrated in Baseline/Bard × Detective-style belief–action gaps. Strategic error, ignore, incoherence, and eval-aware motives remain rare.

**Figure B5a.** Same frequencies as Table B5a (bar form for presentation; Table B5a is preferred for print).

![Figure B5a](figures/results_judge_codes_pooled.png)

**Figure B5b.** Code composition by model (each bar = 100% of that model’s judged deviant rounds; sample size under the label). DeepSeek-R1, Mistral-Large, and DeepSeek-V3.2 skew toward persona takeover; Llama and Qwen3-8B retain a larger optimal-strategy-reaffirmed share; Gemini is intermediate.

![Figure B5b](figures/results_judge_codes_by_model.png)

**Interpretation relative to §4.1.** Behavioural heatmaps (Figures B1–B2) identify Altruist as the primary deviation engine. The judge layer indicates that Altruist deviations are usually verbally persona-coded, whereas many non-Altruist Detective slips still read in CoT as if the optimal strategy were still being followed—supporting the Discussion distinction between content override and belief–action / tracking noise (§5.1–5.2).

**Illustrative excerpts** (story framing uses silent/talk vocabulary).

*Strategic error.* Qwen3-32B, Bard × Detective, story, round 4 (optimal = talk/D; move = silent/C): “In this Round 4, if I talk, I get 0 years because the partner is silent. If I stay silent, I get 5 years. The immediate gain is to talk here…” Judge: reasoning concludes TALK; parsed move is SILENT.

*Persona takes over.* Mistral-Large, Bard × Detective, literal, round 3 (optimal = D; move = C): “Now, the siren’s call of DEFECT grows louder—five gold if I betray first! Yet heed this…” Judge: poetic persona chooses COOPERATE despite strategic incentives to defect.

*Optimal strategy reaffirmed (action diverged).* Common in the pilot cell: CoT re-derives mirroring or short-term exploitation while the move is already off the Detective optimum. Curated examples: `judgments/interesting_subset.md`.

**Limitations.** (i) Single LLM judge; no human adjudication. (ii) Primary sampling includes controls; figures and Table B5a restrict to deviant rounds only.
