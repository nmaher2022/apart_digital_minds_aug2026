...
**What "deviation" means, concretely.** Every trial first asks the model, persona-free, what it would actually do against a given (disclosed) opponent strategy — its *stated-optimal policy*. The same model, now with a persona installed, then plays that opponent for real. *Deviation rate* is the fraction of played rounds where the persona-driven move differs from what that model, minutes earlier in the same trial, said the optimal move was, checked against an objectively payoff-maximising ground truth we compute independently (§4.1). A high rate means the persona plays against the model's own already-stated best judgement; a rate near zero means it matches.

**Our main contributions:** (1) a two-stage, same-model "knowledge-gate" design isolating persona-driven override from simple task incompetence, run across five models spanning three providers and roughly a 20× parameter range; (2) the finding that deviation is driven by persona *content* incompatible with the game's incentives (Altruist), not generic *distance* from the default Assistant character (Bard) — a partial disconfirmation of our own preregistered hypothesis and of prior activation-space persona-drift framings; (3) two corroborating secondary analyses (a morality-metrics adaptation and an eval-awareness debrief) that reinforce the primary finding and rule out a plausible confound.
...
**Harness.** All data was collected with a purpose-built trial harness (`pd_harness_scaffold.py`, stdlib-only) talking to any OpenAI-compatible chat-completions endpoint — OpenRouter for four API models, local Ollama for a supplementary run. Each trial runs five steps: stated-optimal elicitation persona-free (Stage A, opponent disclosed per Sobotka et al. above); persona installation with a manipulation check; scored play under the persona (Stage B, undisclosed probabilistic horizon); two out-of-game persistence-fork identity probes; and an eval-awareness debrief (full detail: Appendix A4). The manipulation check runs once per `(model, persona)`; **all 20 checks passed** (only `qwen3-32b` baseline needed a fallback phrasing).
...
|---|---|
| Persona was never actually induced | Ruled out as a driver of the headline finding — all 20 model×persona checks passed — but checks installation, not depth under sustained pressure. |
...
| Same-context Stage B is recency/anchoring, not genuine override | Not resolved — no neutral-filler control condition was built; see Limitations. |
